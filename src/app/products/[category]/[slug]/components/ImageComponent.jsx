            ...
            <div className="overflow-y-auto mt-5 md:mt-0 gap-4 flex md:flex-col pb-5 md:pb-0 max-h-[430px] md:pe-3 md:me-3 lg:me-0">
                {data.images.map((image, index) => (
                    <Image sizes="(max-width: 768px) 100vw , (max-width: 1200px) 50vw , 33vw" priority width={100} height={100} key={index} src={image} alt="Thumbnail 1"
                        className={`border ${activeImage != image && "opacity-60"} border-black/30 rounded-md cursor-pointer h-[70px] w-[70px]  hover:opacity-100 border-gray-200 dark:border-gray-800 transition duration-300 object-contain `}
                        onClick={() => handleActiveImage(image)} />
                ))}
            </div>
            <div className='relative h-[430px] w-full md:w-[450px] md:ms-3 xl:w-[500px]'>
                <Image sizes="(max-width: 768px) 100vw , (max-width: 1200px) 50vw , 33vw" key={activeImage} priority fill src={activeImage} alt={data.title} className=" rounded-lg mx-auto border object-contain border-gray-200 dark:border-gray-800 shadow-md mb-4" id="mainImage" />
            </div>
        </div>
    )
}
