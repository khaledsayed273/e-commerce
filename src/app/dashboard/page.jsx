...
function Page() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                const response = await fetch(`/api/getStats?timestamp=${timestamp}`);
                if (!response.ok) {
    ...
    useEffect(() => {
        if (analyticsData) {
            setData([
                ...
                {
                    title: "visits",
                    value: analyticsData?.visits?.value,
                    svg: <svg
                        className="fill-primary"
                        width="22"
                        height="22"
                ...
                {
                    title: "totaltime",
                    value: analyticsData?.totaltime?.value,
                    svg: <svg
                        className="fill-primary"
                        width="22"
                        height="18"
        ...
        <main className='min-h-screen'>
            <div className='container mt-10 mx-auto px-2 md:px-5'>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
                    {data.length > 0 ? (
                        data.map((item, index) => (
                            <CardDataStats key={index} title={item.title} total={item.value} rate="0.43%" levelUp>
                                {item.svg}
                            </CardDataStats>
                        ))
                    ) : (
                        skeletons.map((_, index) => (
                            <div
                                key={index}
                                className="rounded-sm border border-stroke bg-white dark:bg-zinc-950 px-7 py-6 shadow-default dark:border-strokedark dark:bg-boxdark animate-pulse"
                            >
                                ...
                                <div className="mt-4 flex items-end justify-between">
                                    <div>
                                        ...
                                        <div className="mt-2 h-4 w-14 bg-gray-300 dark:bg-gray-600 rounded-md"></div>
                                    </div>
                                    <div className="h-5 w-10 bg-gray-300 dark:bg-gray-600 rounded-md"></div>
                                </div>
                            </div>
                        ))
                    )
