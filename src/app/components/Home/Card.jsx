...
const Card = ({ item }) => {
    ...
    return (
        <div className='h-[420px] border shadow-xl rounded-md max-w-[320px] mx-auto my-5 hover:shadow-gray-500 transition-all duration-300'>
            <div onMouseEnter={handleMouseEnter}
                ...
                <Link href={`/products/${item.category}/${item.id}`} className='relative block w-full h-[250px] bg-[#F5F5F5] '>
                    {loading &&
                        ...
                        <Image
                            ...
                            className={`transition-opacity duration-500 ease-in-out ${hovered && item.images.length > 1 ? 'opacity-0' : 'opacity-100'}`}
                        />
                    ...
                    {item.images.length > 1 && (
                        <Suspense fallback={<div className="w-full h-full bg-gray-500 animate-pulse"></div>}>
                            <Image
                                ...
                                alt={item.category}
                                sizes="(max-width: 768px) 33vw, 33vw"
                                className={`absolute top-0 left-0 transition-opacity duration-500 ease-in-out ${hovered ? 'opacity-100' : 'opacity-0'}`}
            ...
            <div className='flex flex-col justify-between mt-3 p-2'>
                <div className="relative inline group max-w-max">
                    <h3 className="font-semibold text-sm md:text-base mb-2 cursor-pointer">
                        {item.title.length <= 24 ? item.title : `${item.title.slice(0, 24)}...`}
                    </h3>
                    <div className="max-w-xs absolute shadow-lg hidden group-hover:block bg-[#f1f1f1] text-black font-semibold px-3 py-[6px] text-[13px] right-0 left-0 mx-auto w-max -bottom-10 rounded before:w-4 before:h-4 before:rotate-45 before:bg-[#dfdfdf] before:absolute before:z-[-1] before:-top-1 before:left-2">
                        {item.title}
                    </div>
                </div>
                <div className='my-4 flex justify-between items-center'>
                    ...
                    <h4 className='font-semibold text-gray-500 text-xs '>{item.brand}</h4>
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <button aria-label="addToCard" onClick={handleOpen} className='p-2 border rounded-md border-gray-400 group'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 transition-all group-hover:scale-75">
